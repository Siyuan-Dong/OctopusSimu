...
import sys
sys.path.append('../')
import TCP_dist.TCP_distribution as tcpdist
import create_error.create_error as cer
...
tst = ["core","time","edge"]
if args.test == 0:
    C_path="../C/core/"+args.algo+"/"+str(args.mem_num)+"/"
elif args.test == 1:
    C_path="../C/time/"
elif args.test == 2:
    C_path="../C/edge/"+str(args.mem_num)+"/"
...
# all_flows = generate_flows(ft, hosts, n_flows,k)
# print("kaka2")
...
for flow_id, flow in all_flows.items():
    arr_dist = constnt
    # arr_dist = partial(expovariate, 1.0/(0.1))
    # arr_dist = partial(tcpdist.TCP_Distribution, time = sec_time,lamda = 0.0001)
    ...
    if ((flow.fid>>80) in culprit_flows) and ((flow.fid>>72)&1):
        ...
        # for switch in culprit_switches[(flow.fid>>80)]:
        for switch in flow.path:
            ...
            # if switch in flow.path:
                ...
                # print("fid",(flow.fid>>80),"interval",arrival_interval)
                break

    # for switch in flow.path:
    #     if switch in culprit_switches:
    #         if culprit_flows[switch] == (flow.fid>>80) and ((flow.fid>>72)&1):
    #             correct_list.append([flow.fid, switch])
    #             # print('interval:',arrival_interval)
    #             break
...
# print("correct_list",len(correct_list),correct_list)
